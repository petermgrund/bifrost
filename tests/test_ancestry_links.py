...
CFG = SyncPaperlessConfig(gramps_id_field_id=5, source_url_field_id=11)
...
class FakeGramps:
    ...
    async def _paged(self, path, page_size=200, **params):
        ...
        return [dict(c) for c in self.citations.values()]
...
def test_scan_lists_missing_links_and_explains_the_rest():
    ...
    assert [(e.action, e.entity, e.source_id, e.gramps_id) for e in items] == [
        ...
        ("failed", "doc", "14", "GONE")]
    assert items[0].detail.startswith("not synced to Gramps yet")
